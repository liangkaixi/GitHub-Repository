...
if __name__ == '__main__':
    ...
    def export_excel():
        ...
        # output the file to the page
        out.put_file('excel.xls', output_file.getvalue(),
                     'application/vnd.ms-excel')

    def main():
        out.put_text('您的表格已经生成！')
        export_excel()
        out.put_buttons([
            {'text': '下载表格', 'value': 'download'},
            {'text': '取消', 'value': 'cancel'}
        ], onclick=download_file)

    def download_file(btn):
        if btn['value'] == 'download':
            out.put_file('excel.xls', get_file_content(),
                         'application/vnd.ms-excel')
        else:
            out.put_text('下载取消！')
